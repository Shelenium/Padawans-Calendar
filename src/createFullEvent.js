...
class CreateFullEvent extends React.Component {
  ...
  render() {
    var loadResources = this.props.event.resources.map(function(item, index) {
    ...
    var loadSpeakers = this.props.event.speakers.map(function(item, index) {
     var mentor = _.find(mentorsListAll, { 'id': item });

      return (
          <div key = { item+index }>
          <Speaker mentor = { item } avatar = { mentorsListAll ? mentor.avatar : '#' } name = {mentorsListAll ? mentor.name : ''} />
          </div>
          ) 
      });
